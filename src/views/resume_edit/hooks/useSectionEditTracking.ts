 ...
 * @example
 * ```tsx
 * const { handleFocus, handleChange } = useSectionEditTracking('experience', 'editor')
 * <input onFocus={(e) => handleFocus(e.target.value)} onChange={(e) => { handleChange(e.target.value); field.onChange(e) }} />
 * ```
 */
export const useSectionEditTracking = (sectionName: SectionEditName, location: SectionEditLocation) => {
  ...
  const handleChange = (nextValue: string) => {
    ...
    amplitude.track(AMPLITUDE_EVENTS.SECTION_EDITED, { section_name: sectionName, location })
